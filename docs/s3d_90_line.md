# s3d_90 라인 (분리됨, 구 v22~v30)

main flow (v1~v21)에서 분리된 9개 카드의 historical record.

## 분리 사유

s3b 69%·s3c 38%가 90% 미달 상태에서 s3d만 fine-tune했던 잘못된 방향성. curriculum 진행 규칙(stage 졸업 = reach_rate ≥ 90%, 다음 카드 우선 대상 = 가장 낮은 미달 stage)에 어긋남 — main flow에서 분리하여 별도 release line·별도 파일로 보존.

main flow의 다음 카드는 새 v22 (s3b 90% 달성)로 새로 시작.

## release rename mapping

| 구 release | 새 release |
|---|---|
| `models-v22` | [`models-s3d_90-v1`](https://github.com/sourceyoo/RL_robot/releases/tag/models-s3d_90-v1) |
| `models-v23-v28` (묶음) | [`models-s3d_90-v2-v7`](https://github.com/sourceyoo/RL_robot/releases/tag/models-s3d_90-v2-v7) |
| `models-v29` | [`models-s3d_90-v8`](https://github.com/sourceyoo/RL_robot/releases/tag/models-s3d_90-v8) |
| `models-v30` | [`models-s3d_90-v9`](https://github.com/sourceyoo/RL_robot/releases/tag/models-s3d_90-v9) |

산문 내부의 `vN` 표기는 historical 그대로 유지 (mapping은 위 표 참조).

## 모델 binary 손실 / 백업

- 구 v23·v24: model.zip 손실 (s3d_arc90/이 그 후 덮임). **tb_logs는 `models-s3d_90-v2-v7` release 포함**.
- 구 v25·v26·v27·v28: 완전 백업 — `models-s3d_90-v2-v7` release + `/tmp/models-v{25,26,27,28}/` 로컬.
- 구 v29 (seed=0/1/2): 완전 백업 — `models-s3d_90-v8` release + `/tmp/models-v29/` 로컬.
- 구 v30 (seed=0/1/2 + `model_best.zip`): 완전 백업 — `models-s3d_90-v9` release + `/tmp/models-v30/` 로컬.

## main flow에 carry-over된 메타 통찰

본 라인에서 발견된 학습 방법론 5개는 main flow에도 적용:

1. **`model_best.zip` 자동 저장 인프라** — peak reach_rate 시점 모델 보존, catastrophic drift 우회. `train.py`·`curriculum.py`에 적용됨.
2. **카드 평가 표준 = multi-seed × best metric** — 단일 seed end는 drift artifact라 noise ±7%p, best metric은 ±4%p.
3. **late catastrophic drift** — 1M long horizon 학습은 peak (~800k) 후 단계적 후퇴 가능. drift trigger 여부는 seed 의존.
4. **yaw reward 형태 sweet spot** — `|·|·0.005` (signed/tanh/YAW_W 변형 모두 동등 또는 미달). 부호 없음이 mode catalysis의 본질.
5. **s3d_90 카드 진짜 천장 ≈ 30%** (best metric 5 seed mean 31.2 ± 4.0%). 단 s3b·s3c 미달 상태의 부수적 정보 — s3c 90% 달성 후에야 본격 의미.

## 함정 (s3d_90 라인 한정)

13. **YAW_W 변형 (구 v23-A·v24-A·v25-A·v27·v28)**: 모두 v22 `|·|`·0.005 미달. **v22가 sweet spot, 단순 변형으론 천장 못 깸**. v23-A peak 41%·v24-A final_dist 0.599m만 통계 유의 (multi-seed 재검증 후보).
14. **단일값 baseline 신뢰 (v29 3-seed로 입증)**: v22 카드 mean 17% ± 2.9%, 5 seed 종합 22% ± 7%p (end 기준). **v22 32%은 +1.4σ outlier**. **카드 비교는 최소 3 seed 평균으로**.
15. **end metric으로 카드 평가 (v30 best-save 3-seed로 입증)**: late catastrophic drift (peak→end −4~−19%p)가 end-100ep을 noise화. **best metric으로 다시 보면 v22 카드 천장 = 29.3 ± 3.3% (v30 3 seed) / 31.2 ± 4.0 (v22+v26+v30 5 seed)**. **카드 평가 표준 = best metric**, end는 안정성·forgetting 진단용으로만.
